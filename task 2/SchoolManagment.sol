...
contract SchoolManagment {
	...
	IERC20 public constant EMCDS = IERC20(0xd9145CCE52D386f254917e481eB44e9943F39138);
	...
	// Emitted when a staff member is suspended.
	event StaffSuspended(address indexed staff, uint256 suspendedAt);
	...
	// Owner can deregister a student, which removes their record and allows them to register again if needed.
	function deregisterStudent(address student) external onlyOwner {
		require(students[student].account != address(0), "Student not registered");

		// Remove student record and update accounts array.
		delete students[student];

		for (uint256 i; i < studentAccounts.length; i++) {
			if (studentAccounts[i] == student) {
	...
	function payStaff(address staff, uint256 amount) external onlyOwner {
		...
		require(!staffs[staff].isSuspended, "Staff is suspended");
	...
	function suspendStaff(address staff) external onlyOwner {
		require(staffs[staff].account != address(0), "Staff not registered");
		staffs[staff].isSuspended = true;

		// Log staff suspension.
		emit StaffSuspended(staff, block.timestamp);
	}
